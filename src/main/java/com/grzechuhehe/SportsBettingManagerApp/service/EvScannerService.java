...
public class EvScannerService {
    ...
    public void scanForEvOpportunities() {
        ...
        try {
            ...
            List<String> sportsToScan = List.of("soccer_epl", "basketball_nba");
            for (String sport : sportsToScan) {
                List<OddsResponseDto> oddsResponses = oddsClient.getOdds(sport, "eu", "h2h");
                log.info("Fetched {} events from TheOddsAPI for sport {}", oddsResponses != null ? oddsResponses.size() : 0, sport);
                
                if (oddsResponses == null || oddsResponses.isEmpty()) {
                    log.warn("No odds fetched from TheOddsAPI for sport {}. Check your API key or sport key.", sport);
                    continue;
                }
                        ...
                        for (OddsResponseDto.MarketDto market : bookmaker.getMarkets()) {
                            for (OddsResponseDto.OutcomeDto outcome : market.getOutcomes()) {
                                ...
                                for (java.util.Map.Entry<String, UnifiedMarketData> entry : unifiedData.entrySet()) {
                                    String entryKey = extractKey(entry.getKey());
                                    if (outcome.getName().toLowerCase().contains(entryKey)) {
